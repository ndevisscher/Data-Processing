...
def scrape_movie_page(dom):
    ...
    writers = ''
    actors = ''
    for a in dom.by_class("itemprop")[:1]: #Title of the movie
        title = plaintext(a.content)
    for b in dom.by_class("infobar"): #Duration of the movie
        for time in b.by_tag("time"):
            duration = plaintext(time.content)
            duration = duration.replace("min",'')
    for c in dom.by_class("infobar"): #Genres of the movie
        for genre in c.by_class("itemprop"):
            genres = genres + ';' + plaintext(genre.content)
    for d in dom.by_class("txt-block")[:1]: #Directors of the movie
        for director in d.by_class("itemprop"):
            directors = directors + ';' + plaintext(director.content)
    for e in dom.by_class("txt-block")[1:2]: #Writers of the movie
        for writer in e.by_class("itemprop"):
            writers = writers + ';' + plaintext(writer.content)
    for f in dom.by_class("txt-block")[2:3]: #actors of the movie
        for actor in f.by_class("itemprop")[:3]:
            actors = actors + ';' + plaintext(actor.content)    
    for g in dom.by_class("star-box-details"):#Rating of the movie
        rating = plaintext(g[1].content)
    for h in dom.by_class("star-box-details"):#Rating of the movie
        n_ratings = plaintext(g[4].content)
        n_ratings = n_ratings.replace("users",'')
            
    # Return everything of interest for this movie (all strings as specified
    # in the docstring of this function).
    title = title.encode("ascii","ignore")
    genres = genres[1:]
    directors = directors[1:].encode("ascii","ignore")
    writers = writers[1:].encode("ascii","ignore")
